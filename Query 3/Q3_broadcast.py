...
crime_geo_df = crime_df.join(geocode_df.hint("BROADCAST"), (crime_df["LAT"] == geocode_df["LAT"]) & (crime_df["LON"] == geocode_df["LON"]), "inner").select(crime_df["Vict Descent"], geocode_df["ZIPcode"])

#Find LA city zip codes
la_zips = income_df.join(crime_geo_df.hint("BROADCAST"), crime_geo_df["ZIPcode"] == income_df["Zip Code"], "left_semi")
...
crimes_top3 = crime_geo_df.join(top3.hint("BROADCAST"), crime_geo_df['ZIPcode'] == top3['Zip Code'], 'left_semi')
crimes_bottom3 = crime_geo_df.join(bottom3.hint("BROADCAST"), crime_geo_df['ZIPcode'] == bottom3['Zip Code'], 'left_semi')
...
crimes_top3 = crimes_top3_grouped.join(descent_code_df.hint("BROADCAST"), on="Vict Descent", how="left")
crimes_bottom3 = crimes_bottom3_grouped.join(descent_code_df.hint("BROADCAST"), on="Vict Descent", how= "left")
